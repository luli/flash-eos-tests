...
#include "constants.h"
  ...
  use Hydro_data,           ONLY: hy_smallpres
  ...
  if (hy_3Ttry_D==3.0) then
     ...
     if (pele_adv <= 0.0 .and. pion_adv >=0.0) then
        PeP = hy_smallpres / (pele_adv + pion_adv + prad_adv)
        PiP = 1.0 - PeP - PrP
     else if  (pele_adv >= 0.0 .and. pion_adv <=0.0) then
        PiP = hy_smallpres / (pele_adv + pion_adv + prad_adv)
        PeP = 1.0 - PiP - PrP
     else
